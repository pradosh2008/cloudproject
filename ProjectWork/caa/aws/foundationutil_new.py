...
source_bucket_name="asc-aadpdatalakelayer"
...
# def write_partitioned_sink(target_gdf,target_tbl,partitionKeys,glueContext,source_prefix):
    # sink_gdf = glueContext.write_dynamic_frame.from_options(frame = target_gdf, connection_type = "s3", connection_options = {"path": "s3://"+source_bucket_name+source_prefix+"//"+target_tbl, "partitionKeys": partitionKeys}, format = "parquet", transformation_ctx = "sink_gdf")


# def write_nonpartitioned_sink(base_gdf,target_tbl,glueContext,source_prefix):
    # sink_gdf = glueContext.write_dynamic_frame.from_options(frame = base_gdf, connection_type = "s3", connection_options = {"path": "s3://"+source_bucket_name+source_prefix+"//"+target_tbl}, format = "parquet", transformation_ctx = "sink_gdf")
...
def write_partitioned_sink(target_gdf,target_tbl,partitionKeys,glueContext,source_prefix):
    path="s3://"+source_bucket_name+source_prefix+"//"+target_tbl
    sink = glueContext.getSink(connection_type="s3", path=path,\
...
# def write_nonpartitioned_sink(target_gdf,target_tbl,glueContext,source_prefix):
    # path="s3://"+source_bucket_name+source_prefix+"//"+target_tbl
    # sink = glueContext.getSink(connection_type="s3", path=path,\
              # enableUpdateCatalog=True,\
              # updateBehavior="UPDATE_IN_DATABASE"\
