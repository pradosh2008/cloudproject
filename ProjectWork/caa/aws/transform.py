...
def write_partitioned_sink(ordered_gdf,target_tbl,source_tbl,glueContext):
    sink_gdf = glueContext.write_dynamic_frame.from_options(frame = ordered_gdf, connection_type = "s3", connection_options = {"path": "s3://aadp-datalakelayer"+"//"+target_tbl, "partitionKeys": [source_tbl + '_year']}, format = "parquet", transformation_ctx = "sink_gdf")


def write_nonpartitioned_sink(ordered_gdf,target_tbl,glueContext):
    sink_gdf = glueContext.write_dynamic_frame.from_options(frame = ordered_gdf, connection_type = "s3", connection_options = {"path": "s3://aadp-datalakelayer"+"//"+target_tbl}, format = "parquet", transformation_ctx = "sink_gdf")
