...
drop TABLE ascena_analytic_mart_dev.PROMO_OFFER_LINKAGE ;

CREATE TABLE IF NOT EXISTS ascena_analytic_mart_dev.PROMO_OFFER_LINKAGE (
...
INSERT OVERWRITE TABLE ascena_analytic_mart_dev.PROMO_OFFER_LINKAGE
...
FROM ascena_staging_dev.jus_hst_promotions_drc AS HPO
LEFT JOIN  ascena_staging_dev.JUS_HST_PROMOTION_SEGMENTS_DRC AS HPS
	ON HPO.PROMOTION_ID = HPS.PROMOTION_ID
LEFT JOIN ascena_staging_dev.JUS_HST_PROMOTION_CELLS_DRC AS HPC 
...
LEFT JOIN ascena_staging_dev.JUS_HST_PACKAGES_DRC AS HP
	ON HP.PACKAGE_ID = HPC.PACKAGE_ID 
LEFT JOIN ascena_staging_dev.jus_hst_package_treatments_drc AS HPT
	ON HP.PACKAGE_ID  = HPT.PACKAGE_ID 
LEFT JOIN ascena_staging_dev.JUS_HST_TREATMENTS_DRC AS HT
	ON HT.TREATMENT_CODE = HPT.TREATMENT_CODE
LEFT JOIN ascena_staging_dev.jus_hst_offer_codes_drc AS HO
