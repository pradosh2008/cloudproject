--/*********************************************************************
--* 
--* Script Name: DIM_STORE table creation and data population
--*
--**********************************************************************/
...
,batch_id
from (select 
md5(concat('JUS|',coalesce(e.store_id,''))) as store_key
,'JUS' as brand_cd
...
,row_number() over (partition by e.store_no 
order by from_unixtime(UNIX_TIMESTAMP(e.end_effective_date, "MM/dd/yyyy HH:mm:ss")) desc
)  rn
from ASCENA_STAGING_DEV.jus_hst_stores_drc as e)a
where rn = 1;

--Primary Key check
--select store_key from ascena_outbound_dev.DIM_STORE group by store_key having count(*)>1;
