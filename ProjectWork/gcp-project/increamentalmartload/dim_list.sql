...
drop TABLE ascena_analytic_mart_dev.dim_list;

CREATE TABLE IF NOT EXISTS ascena_analytic_mart_dev.dim_list (
LIST_KEY CHAR(32)
, list_id BIGINT
, list_type_cd STRING
, list_desc STRING
...
INSERT OVERWRITE TABLE ascena_analytic_mart_dev.dim_list
SELECT 
...
,batch_id
FROM ascena_staging_dev.jus_hst_lists_drc  AS hl 
UNION
SELECT 
...
FROM ASCENA_STAGING_DEV.jus_hst_customer_lists_drc   AS hcl
left join ascena_staging_dev.jus_hst_lists_drc hl
on hcl.list_id = hl.list_id
where hl.list_id is null
;
