--/*********************************************************************
--* 
--* Script Name: BRAND_ITEM table creation and data population
--*
--**********************************************************************/
--/*--
...
CREATE TABLE IF NOT EXISTS ascena_analytic_mart_dev.DIM_ITEM 
(
     item_key  char(32)
    ,brand_cd               string
    ,short_sku_id INT
...
INSERT OVERWRITE TABLE ascena_analytic_mart_dev.DIM_ITEM 
...
--Primary Key check
--SELECT SKU_KEY, COUNT(*) FROM ascena_analytic_mart_dev.DIM_ITEM GROUP BY SKU_KEY HAVING COUNT(SKU_KEY)>1;
