...
drop table ascena_analytic_mart_dev.DIM_BRAND_CUSTOMER_ADDRESS ;
CREATE TABLE IF NOT EXISTS ascena_analytic_mart_dev.DIM_BRAND_CUSTOMER_ADDRESS (
...
create temporary table ASCENA_STAGING_DEV.jus_hst_customer_addresses_tmp as
    select INDIVIDUAL_ID
           ...
           ,ZIP_4
           ,BATCH_ID
    from ASCENA_STAGING_DEV.jus_hst_customer_addresses_drc;
	

--Brand customer key & Address key make up the primary key
INSERT OVERWRITE TABLE ascena_analytic_mart_dev.DIM_BRAND_CUSTOMER_ADDRESS
...
, a.batch_id
FROM ascena_staging_dev.jus_hst_customer_addresses_dr as a
 join ASCENA_STAGING_DEV.jus_hst_customer_addresses_tmp as f
 on coalesce(a.individual_id,'')=coalesce(f.individual_id, '')
 and coalesce(upper(trim(a.address_1)),'')=coalesce(upper(trim(f.address_1)),'')
 and coalesce(upper(trim(a.address_2)),'')=coalesce(upper(trim(f.address_2)),'')
 and coalesce(upper(trim(a.city)),'')=coalesce(upper(trim(f.city)),'')
 and coalesce(upper(trim(a.state_province)),'')=coalesce(upper(trim(f.state_province)),'')
 and coalesce(upper(trim(a.zip_postal_code)),'')=coalesce(upper(trim(f.zip_postal_code)),'')
 and coalesce(upper(trim(a.zip_4)),'')=coalesce(upper(trim(f.zip_4)),'')
