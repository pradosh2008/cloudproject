...
create TABLE IF NOT EXISTS ascena_analytic_mart_dev.DIM_BRAND_CUSTOMER_EMAIL_MAY 
(
...
INSERT OVERWRITE TABLE ascena_analytic_mart_dev.dim_brand_customer_email_may 
select
...
from (select md5(concat('JUS|',coalesce(e.individual_id,''),'|', coalesce(upper(trim(e.email_address )),''))) as brand_customer_email_key
    ...
    from ASCENA_STAGING_DEV.jus_hst_customer_email_addresses_dr as e where e.batch_id='20190501'
) as m
where rn=1
;
