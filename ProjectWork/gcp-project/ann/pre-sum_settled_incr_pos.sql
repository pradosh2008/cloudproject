...
join 
		...
		AND interface_control_flag != 20 ) as inc  --header interface control flag logic
					JOIN edl_landing.ann_ann_aw_transactions_line l
					 ...
					 left JOIN edl_landing.ann_calendar cal
						  ON EXTRACT (DATE FROM (PARSE_DATETIME('%m/%d/%Y %H:%M:%S',
		entry_date_time)))  = CAST (day_dt AS date)
             left JOIN (
							   SELECT	*
								 FROM	(
										  (
                ...
                /* CONCAT(CAST(inc.store_no AS string)	,':'
					...
					NOT IN 
							...
							where inc_20.interface_control_flag  = 20) and	*/		-- old logic for interface control flag	
 ...
 join 
		...
		SELECT						
				...
				,cast(pr.style_nbr as string) hist_Style_ID
...
where	bi.store_id not in ( 611, 612, 613, 616, 617, 618 , 619)  
	and bi.week_id > 201830
group by 1
--,2,3,4
--having 
--abs(sum(BI.Net_Sales_Unit_cnt- incr.Net_Sales_Unit_cnt))  > 0 
--abs(sum(BI.Net_Sales_Unit_cnt- hist.Net_Sales_Unit_cnt))  >0
